...
static int32_t maPlaySound(AudioSystem* audio, int32_t soundIndex, int32_t priority, bool loop) {
    ...
    if (isStream) {
        ...
        if (inAudo) {
            ...
            int statResult = stat(path, &audioStat);

            if (statResult == 0) {
                logInfo("Audio: external file EXISTS '%s' size=%lld mode=%o\\n",
                        path,
                        (long long)audioStat.st_size,
                        (unsigned)(audioStat.st_mode & 0777));
            } else {
                logWarn("Audio: external file MISSING '%s' errno=%d\\n",
                        path, errno);
            }
