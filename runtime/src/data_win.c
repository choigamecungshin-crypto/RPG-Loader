...
static void parseFONT(BinaryReader* reader, DataWin* dw) {
    ...
    repeat(count, i) {
        ...
        logInfo("FONT DEBUG: name=%s display=%s tpag=%d spriteFont=%d spriteIndex=%d glyphs=%u scale=(%g,%g) asc=%d maxH=%u",
            font->name ? font->name : "?",
            ...
            (double) font->scaleY,
            font->ascenderOffset,
            (unsigned) font->maxGlyphHeight);

        for (uint32_t dbg = 0; dbg < font->glyphCount && dbg < 8; dbg++) {
            FontGlyph* g = &font->glyphs[dbg];
            logInfo("FONT GLYPH[%u]: ch=%u src=(%u,%u %ux%u) shift=%d offset=%d",
                dbg,
                (unsigned) g->character,
                (unsigned) g->sourceX,
                (unsigned) g->sourceY,
                (unsigned) g->sourceWidth,
                (unsigned) g->sourceHeight,
                (int) g->shift,
                (int) g->offset);
        }
...
static void resolveAllTPAGReferences(BinaryReader* reader, DataWin* dw, uint32_t* ptrs, uint32_t count) {
    ...
    repeat(dw->font.count, i) {
        ...
        uint32_t oldOffset = (uint32_t) fnt->tpagIndex;
        fnt->tpagIndex = findTPAGIndexByOffset(reader, dw, ptrs, count, oldOffset);

        logInfo("FONT TPAG RESOLVE: name=%s offset=%u -> index=%d / tpagCount=%u",
            fnt->name ? fnt->name : "?",
            (unsigned) oldOffset,
            fnt->tpagIndex,
            (unsigned) dw->tpag.count);
    }
    }
}
