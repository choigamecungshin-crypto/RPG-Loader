...
static void flushBatch(GLRenderer* gl) {
    ...
    if (gl->base.currentShader != -1) {
        ...
        GLint fontMaskLocation = glGetUniformLocation(
            shader->shaderId,
            "uFontMask"
        );
        if (fontMaskLocation >= 0) {
            glUniform1i(
                fontMaskLocation,
                modernGl->batchFontMask ? 1 : 0
            );
        }
        ...
        if (uniform != nullptr) {
            ...
            static int texBindDebugCount = 0;
            if (texBindDebugCount < 30) {
                logInfo("TEXBIND GML shader=%d tex=%u unit=%d\\n",
                    gl->base.currentShader,
                    (unsigned)modernGl->currentTextureId,
                    uniform->samplerSlot);
                texBindDebugCount++;
            }
        }
        ...
        if (modernGl->defaultShaderProgram != nullptr) {
            GLint fontMaskLocation = glGetUniformLocation(
                modernGl->defaultShaderProgram->shaderId,
                "uFontMask"
            );
            if (fontMaskLocation >= 0) {
                glUniform1i(
        ...
        static int defaultStateDebugCount = 0;
        if (defaultStateDebugCount < 30) {
            ...
            sampler = glGetUniformLocation(
                modernGl->defaultShaderProgram->shaderId,
                "uTexture"
            );

            GLint samplerValue = -1;
            if (sampler >= 0) {
                glGetUniformiv(
                    modernGl->defaultShaderProgram->shaderId,
                    sampler,
            ...
            logInfo(
                "DEFAULTGL program=%d expected=%u active=%d boundTex=%d "
                "currentTex=%u samplerLoc=%d samplerUnit=%d\\n",
                currentProgram,
                (unsigned)modernGl->defaultShaderProgram->shaderId,
    ...
    if (vertexDebugCount < 20 && modernGl->batchType == BATCHTYPE_QUAD) {
        GlVertex *v = gl->vertexData;

        logInfo(
            "VERTEXDEBUG count=%d "
            "V0=(%.2f,%.2f uv=%.5f,%.5f rgba=%u,%u,%u,%u) "
            "V1=(%.2f,%.2f uv=%.5f,%.5f rgba=%u,%u,%u,%u) "
            "V2=(%.2f,%.2f uv=%.5f,%.5f rgba=%u,%u,%u,%u) "
            "V3=(%.2f,%.2f uv=%.5f,%.5f rgba=%u,%u,%u,%u)\\n",
            modernGl->batchCount,
    ...
    if (modernGl->batchType == BATCHTYPE_QUAD) {
        ...
        if (drawDebugCount < 30) {
            ...
            glActiveTexture(GL_TEXTURE0 + (activeTex - GL_TEXTURE0));
            ...
            logInfo(
                "DRAWDEBUG batch=%d count=%d program=%d tex=%d currentTex=%u "
                "active=%d unit0=%d sampler=%d/%d filter=%d/%d wrap=%d/%d "
                "fontMask=%d shader=%d\\n",
                modernGl->batchType,
                modernGl->batchCount,
                program,
        ...
        static int blendStateDebugCount = 0;
        if (blendStateDebugCount < 100) {
            ...
            logInfo(
                "GLSTATE blend=%d srcRGB=%d dstRGB=%d eqRGB=%d "
                "program=%d tex=%d batch=%d fontMask=%d shader=%d\n",
                blend ? 1 : 0,
                srcRGB,
                dstRGB,
                eqRGB,
...
static void glShaderSettingsRefresh(Renderer* renderer) {
    ...
    if (renderer->currentShader != -1) {
        ...
        glUniform1i(modernGl->uTexture->location, 0);
    }
}
...
static bool debugDrawTPAG6(GLRenderer* gl);
...
static void glEndFrameEnd(Renderer* renderer) {
    ...
    if (modernGl->isGL3) {
        ...
        GLCommon_endLetterboxBlit(
            gl->surfaceWidth[appId],
            gl->surfaceHeight[appId],
            gl->gameW,
            gl->gameH,
            gl->windowW,
            gl->windowH,
            modernGl->hostFramebuffer
        );
    } else {
        glBindFramebuffer(GL_FRAMEBUFFER, modernGl->hostFramebuffer);
...
bool GLRenderer_ensureTextureLoaded(GLRenderer* gl, uint32_t pageId) {
    ...
    if (pageId == 22 && txtr->blobData && txtr->blobSize > 0) {
        logInfo("TXTRRAW PAGE=22 SIZE=%zu "
                "HEAD=%02X%02X%02X%02X%02X%02X%02X%02X "
                "TAIL=%02X%02X%02X%02X%02X%02X%02X%02X\\n",
                (size_t)txtr->blobSize,
                txtr->blobData[0],
                txtr->blobData[1],
                txtr->blobData[2],
                txtr->blobData[3],
                ...
                txtr->blobData[6],
                txtr->blobData[7],
                txtr->blobData[txtr->blobSize - 8],
                txtr->blobData[txtr->blobSize - 7],
                txtr->blobData[txtr->blobSize - 6],
                txtr->blobData[txtr->blobSize - 5],
                txtr->blobData[txtr->blobSize - 4],
                txtr->blobData[txtr->blobSize - 3],
                txtr->blobData[txtr->blobSize - 2],
                txtr->blobData[txtr->blobSize - 1]);
    }
    ...
    if (pageId == 22) {
        logInfo(
            "TXTRDECODE page=%u gm2022_5=%d blobSize=%zu "
            "HEAD=%02X%02X%02X%02X%02X%02X%02X%02X\n",
            pageId,
            gm2022_5 ? 1 : 0,
            (size_t)txtr->blobSize,
            txtr->blobSize > 0 ? txtr->blobData[0] : 0,
            txtr->blobSize > 1 ? txtr->blobData[1] : 0,
            txtr->blobSize > 2 ? txtr->blobData[2] : 0,
            txtr->blobSize > 3 ? txtr->blobData[3] : 0,
            txtr->blobSize > 4 ? txtr->blobData[4] : 0,
            txtr->blobSize > 5 ? txtr->blobData[5] : 0,
            txtr->blobSize > 6 ? txtr->blobData[6] : 0,
            txtr->blobSize > 7 ? txtr->blobData[7] : 0
        );
    }
    ...
    if (w > 0 && h > 0) {
        size_t p0 = 0;
        size_t p1 = ((size_t)w - 1) * 4;
        size_t p2 = ((size_t)(h - 1) * (size_t)w) * 4;
        size_t p3 = (((size_t)(h - 1) * (size_t)w) + (size_t)(w - 1)) * 4;

        logInfo(
            "TXTR %u corners: TL=%02X%02X%02X%02X "
            "TR=%02X%02X%02X%02X "
            "BL=%02X%02X%02X%02X "
            "BR=%02X%02X%02X%02X\n",
            pageId,
            pixels[p0], pixels[p0+1], pixels[p0+2], pixels[p0+3],
            pixels[p1], pixels[p1+1], pixels[p1+2], pixels[p1+3],
            pixels[p2], pixels[p2+1], pixels[p2+2], pixels[p2+3],
            pixels[p3], pixels[p3+1], pixels[p3+2], pixels[p3+3]
        );
    }
    ...
    /*
     * DEBUG: inspect actual pixels inside TPAG 6 and 7.
     */
    if (pixels && w > 0 && h > 0 && pageId == 22) {
        uint32_t debugIds[] = {6, 7};

        for (int di = 0; di < 2; di++) {
            uint32_t ti = debugIds[di];

            if (ti >= dw->tpag.count)
                continue;

            TexturePageItem* dbg = &dw->tpag.items[ti];

            if (dbg->texturePageId != (int16_t)pageId)
                continue;

            uint32_t x0 = dbg->sourceX;
            uint32_t y0 = dbg->sourceY;
            uint32_t x1 = x0 + dbg->sourceWidth;
            uint32_t y1 = y0 + dbg->sourceHeight;

            if (x1 > (uint32_t)w || y1 > (uint32_t)h)
                continue;
            ...
            for (uint32_t y = y0; y < y1; y++) {
                for (uint32_t x = x0; x < x1; x++) {
                    size_t p0 = ((size_t)y * (size_t)w + x) * 4;

                    uint8_t r = pixels[p0];
                    uint8_t g = pixels[p0 + 1];
                    uint8_t b = pixels[p0 + 2];
                    uint8_t al = pixels[p0 + 3];
            ...
            logInfo(
                "TPAGSTAT ti=%u SRC=%u,%u %ux%u "
                "OPAQUE=%u TRANSPARENT=%u BLACK=%u NONBLACK=%u "
                "RGB_MIN=%02X%02X%02X RGB_MAX=%02X%02X%02X\n",
                ti,
                x0, y0,
                dbg->sourceWidth,
                dbg->sourceHeight,
    ...
    // Android/GLES: make the pixel upload alignment explicit.
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

    static int uploadDebugCount = 0;
    if (uploadDebugCount < 20) {
        logInfo(
            "TEXUPLOAD page=%u id=%u size=%dx%d "
            "P0=%02X%02X%02X%02X P1=%02X%02X%02X%02X\n",
            pageId,
            (unsigned)gl->glTextures[pageId],
            w, h,
    ...
    if (texErr != GL_NO_ERROR) {
        logWarn("TEXUPLOAD ERROR page=%u error=0x%04X\n",
                pageId, texErr);
    }
...
static void setFontMaskBatchState(GLRenderer* gl, bool enabled) {
    GLModernRenderer* modernGl = (GLModernRenderer*) gl;

    if (modernGl->batchCount != 0 &&
        modernGl->batchFontMask != enabled) {
        flushBatch(gl);
    }
    ...
    /*
     * GameMaker font pages are alpha masks.
     * They require normal alpha blending even when the
     * surrounding draw state has blending disabled.
     */
    if (enabled) {
        glEnable(GL_BLEND);
        glBlendEquation(GL_FUNC_ADD);
...
static void emitTexturedQuad(
    ...
    static int quadAlphaDebug = 0;
    if (quadAlphaDebug < 30) {
        logInfo(
            "QUADALPHA tex=%u color=%u,%u,%u alpha=%u "
            "uv=%.5f,%.5f -> %.5f,%.5f\\n",
            (unsigned)texId,
...
static void glDrawSprite(Renderer* renderer, int32_t tpagIndex, float x, float y, float originX, float originY, float xscale, float yscale, float angleDeg, uint32_t color, float alpha) {
    ...
    if (spriteDebugCount < 1000) {
        logInfo(
            "SPRITE TPAG=%d PAGE=%d SRC=%u,%u %ux%u "
            "TARGET=%u,%u %ux%u TEX=%dx%d\\n",
            tpagIndex,
            (int)tpag->texturePageId,
            (unsigned)tpag->sourceX,
            (unsigned)tpag->sourceY,
            (unsigned)tpag->sourceWidth,
            (unsigned)tpag->sourceHeight,
            (unsigned)tpag->targetX,
            (unsigned)tpag->targetY,
            (unsigned)tpag->targetWidth,
            (unsigned)tpag->targetHeight,
            texW,
            texH
        );
        spriteDebugCount++;
...
static void glDrawSpritePartColor(Renderer* renderer, int32_t tpagIndex, int32_t srcOffX, int32_t srcOffY, int32_t srcW, int32_t srcH, float x, float y, float xscale, float yscale, float angleDeg, float pivotX, float pivotY, uint32_t color1, uint32_t color2, uint32_t color3, uint32_t color4, float alpha) {
    ...
    TexturePageItem* tpag = &dw->tpag.items[tpagIndex];
    static int spritePartDebugCount = 0;
    if (spritePartDebugCount < 300) {
        logInfo(
            "SPRITE_PART TPAG=%d PAGE=%d "
            "ATLAS=%u,%u %ux%u "
            "PART=%d,%d %dx%d "
            "POS=%.1f,%.1f SCALE=%.2f,%.2f "
            "COLORS=%08X,%08X,%08X,%08X ALPHA=%.3f\n",
            tpagIndex,
            (int)tpag->texturePageId,
            (unsigned)tpag->sourceX,
            (unsigned)tpag->sourceY,
...
static void glDrawSpritePos(Renderer* renderer, int32_t tpagIndex, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float alpha) {
    ...
    static int spritePosDebugCount = 0;
    if (spritePosDebugCount < 100) {
        logInfo(
            "SPRITE_POS TPAG=%d PAGE=%d SRC=%u,%u %ux%u "
...
static bool glResolveFontState(GLRenderer* gl, DataWin* dw, Font* font, GlFontState* state) {
    ...
    if (!font->isSpriteFont) {
        ...
        logInfo("FONT TEX DEBUG: name=%s page=%d tex=%u size=%ux%u",
            font->name ? font->name : "?",
            (int) pageId,
            (unsigned) state->texId,
            (unsigned) state->texW,
            (unsigned) state->texH);
...
static bool glResolveGlyph(GLRenderer* gl, DataWin* dw, GlFontState* state, FontGlyph* glyph, float cursorX, float cursorY, GLuint* outTexId, float* outU0, float* outV0, float* outU1, float* outV1, float* outLocalX0, float* outLocalY0) {
    ...
    if (font->isSpriteFont && state->spriteFontSprite != nullptr) {
        ...
        *outLocalY0 = cursorY + (float) (int32_t) glyphTpag->targetY - (float) font->spriteOriginYAdjust;
        ...
        static bool fontUvDebugPrinted = false;
        if (!fontUvDebugPrinted && font->name && strcmp(font->name, "fnt_maintext") == 0) {
            logInfo("FONT UV DEBUG: name=%s tpagSrc=(%u,%u %ux%u) glyphSrc=(%u,%u %ux%u) tex=%ux%u",
                font->name,
                (unsigned) state->fontTpag->sourceX,
                (unsigned) state->fontTpag->sourceY,
                (unsigned) state->fontTpag->sourceWidth,
                (unsigned) state->fontTpag->sourceHeight,
                (unsigned) glyph->sourceX,
                (unsigned) glyph->sourceY,
                (unsigned) glyph->sourceWidth,
                (unsigned) glyph->sourceHeight,
                (unsigned) state->texW,
                (unsigned) state->texH);

            fontUvDebugPrinted = true;
        }
...
static bool glSetRenderTarget(Renderer* renderer, int32_t surfaceId, bool implicitApplicationSurface) {
    ...
    if (surfaceId == renderer->runner->applicationSurfaceId && implicitApplicationSurface) {
        ...
        logInfo(
            "CAMERADEBUG: game=%dx%d window=%dx%d "
            "surface=%dx%d CPort=%d,%d,%d,%d "
            "view=%.2f,%.2f %.2fx%.2f "
            "angle=%.2f\n",
            gl->gameW,
            gl->gameH,
...
static void glSurfaceCopy(Renderer* renderer, int32_t destSurfaceID, int32_t destX, int32_t destY, int32_t srcSurfaceID, int32_t srcX, int32_t srcY, int32_t srcW, int32_t srcH, bool part) {
    ...
    if (modernGl->isGL3) {
        ...
        gl->base.vtable->setGuiProjection(renderer, gl->surfaceWidth[destSurfaceID], gl->surfaceHeight[destSurfaceID], gl->surfaceWidth[destSurfaceID], gl->surfaceHeight[destSurfaceID], true);
...
static void glGpuSetBlendMode(Renderer* renderer, int32_t mode) {
    ...
    static int blendDebugCount = 0;
    if (blendDebugCount < 100) {
        logInfo("BLEND mode=%d src=%d dst=%d enabled=%d\\n",
            mode,
            gl->currentSFactor,
            gl->currentDFactor,
            gl->blendEnable ? 1 : 0);
