...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, MAYBE_UNUSED void* reserved) {
    ...
    jclass localCls = (*env)->FindClass(env, "com/rpgloader/butterscotch/ButterscotchNative");
    if (localCls == nullptr) {
...
#define JNI_FN(name) Java_com_rpgloader_butterscotch_ButterscotchNative_##name
...
static void thumbnailPngWrite(
    void* context,
    void* data,
    int size
) {
    ThumbnailPngBuffer* buffer =
        (ThumbnailPngBuffer*)context;
    ...
    if (needed > buffer->capacity) {
        size_t newCapacity =
            buffer->capacity ? buffer->capacity : 4096;
...
JNIEXPORT jbyteArray JNICALL JNI_FN(dataWinThumbnailPng)(
    JNIEnv* env,
    MAYBE_UNUSED jclass cls,
    jlong handle
) {
    DataWin* dw = (DataWin*) (uintptr_t) handle;

    if (dw == nullptr) {
        return nullptr;
    }

    if (dw->sprt.sprites == nullptr ||
        dw->sprt.count == 0 ||
        dw->tpag.items == nullptr ||
        dw->tpag.count == 0 ||
        dw->txtr.textures == nullptr ||
        dw->txtr.count == 0) {
        return nullptr;
    }

    /*
     * Find the first sprite that actually has a valid texture frame.
     * SPRT -> TPAG -> TXTR
     */
    Sprite* sprite = nullptr;
    TexturePageItem* item = nullptr;

    for (uint32_t si = 0; si < dw->sprt.count; si++) {
        Sprite* candidate = &dw->sprt.sprites[si];

        if (!candidate->present ||
            candidate->textureCount == 0 ||
            candidate->tpagIndices == nullptr) {
            continue;
        }

        for (uint32_t fi = 0; fi < candidate->textureCount; fi++) {
            int32_t tpagIndex = candidate->tpagIndices[fi];
            ...
            TexturePageItem* candidateItem =
                &dw->tpag.items[tpagIndex];

            if (!candidateItem->present ||
                candidateItem->texturePageId < 0 ||
                (uint32_t)candidateItem->texturePageId >= dw->txtr.count) {
                continue;
            }

            if (candidateItem->sourceWidth == 0 ||
                candidateItem->sourceHeight == 0) {
                continue;
            }
    ...
    if (sprite == nullptr || item == nullptr) {
        return nullptr;
    }

    uint32_t textureId = (uint32_t)item->texturePageId;

    DataWin_loadTxtrIfNeeded(dw, textureId);

    Texture* texture = &dw->txtr.textures[textureId];
    ...
    uint8_t* rgba = ImageDecoder_decodeToRgba(
        texture->blobData,
        texture->blobSize,
        gm2022_5,
        &texW,
    ...
    if (rgba == nullptr || texW <= 0 || texH <= 0) {
        return nullptr;
    }

    int sx = (int)item->sourceX;
    int sy = (int)item->sourceY;
    int sw = (int)item->sourceWidth;
    int sh = (int)item->sourceHeight;

    /*
     * Some files contain invalid/out-of-range TPAG entries.
     */
    if (sx < 0 || sy < 0 ||
        sx >= texW || sy >= texH ||
        sw <= 0 || sh <= 0) {
    ...
    /*
     * Limit library thumbnails so a giant texture page cannot
     * create a huge PNG.
     */
    ...
    if (outW > maxSize || outH > maxSize) {
        ...
        if (outW > maxSize) {
            scale = (float)maxSize / (float)outW;
        }

        if ((float)outH * scale > maxSize) {
            scale = (float)maxSize / (float)outH;
        }
    ...
    size_t cropSize =
        (size_t)outW * (size_t)outH * 4;

    uint8_t* crop =
        (uint8_t*)malloc(cropSize);
    ...
    /*
     * Nearest-neighbour resize/crop.
     * Thumbnails are tiny, so this keeps implementation simple.
     */
    for (int y = 0; y < outH; y++) {
        int srcY =
            sy + (int)((long long)y * sh / outH);

        uint8_t* dstRow =
            crop + (size_t)y * (size_t)outW * 4;

        for (int x = 0; x < outW; x++) {
            int srcX =
                sx + (int)((long long)x * sw / outW);

            const uint8_t* src =
                rgba +
                ((size_t)srcY * (size_t)texW +
                 (size_t)srcX) * 4;
    ...
    if (!ok || png.data == nullptr || png.size == 0) {
        free(png.data);
        return nullptr;
    }

    if (png.size > INT32_MAX) {
        free(png.data);
        return nullptr;
    }
...
JNIEXPORT void JNICALL JNI_FN(onKeyDown)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls, jint keyCode) {
    ...
    logInfo("ANDROID KEY DOWN key=%d state=%d pressed=%d\\n",
            keyCode,
            RunnerKeyboard_check(runner->keyboard, keyCode),
            RunnerKeyboard_checkPressed(runner->keyboard, keyCode));
}
...
JNIEXPORT void JNICALL JNI_FN(onKeyUp)(MAYBE_UNUSED JNIEnv* env, MAYBE_UNUSED jclass cls, jint keyCode) {
    ...
    logInfo("ANDROID KEY UP key=%d state=%d released=%d\\n",
            keyCode,
            RunnerKeyboard_check(runner->keyboard, keyCode),
            RunnerKeyboard_checkReleased(runner->keyboard, keyCode));
}
